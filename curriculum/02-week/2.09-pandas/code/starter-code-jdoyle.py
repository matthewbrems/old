...
users.index             # "the index" (aka "the labels") -- ATTRIBUTE -- no parameters, no arguments to be fed
users.columns           # column names (which is "an index")
users.dtypes            # data types of each column
users.shape             # number of rows and columns
users.values            # underlying numpy array; pandas stored this way for efficiency
...
drinks_URL = 'https://raw.githubusercontent.com/josephnelson93/GA-DSI/master/example-lessons/plotting-with-pandas/drinks.csv'
...
freq_occupation = users['occupation'].value_counts()
top_occupations = freq_occupation.sort_values(ascending=False)[0:3]

# BONUS: create the 'users' DataFrame from the u.user_original file (which lacks a header row)
URL = 'https://raw.githubusercontent.com/justmarkham/DAT8/master/data/u.user_original'
...
users[young_bool]                   # ...and use that Series to filter rows >> data frame with specific condition met
users[users.age < 20]               # or, combine into a single step
users[users.age < 20].occupation    # select one column from the filtered results
users[users.age < 20].occupation.value_counts()     # value_counts of resulting Series
users[young_bool].describe
...
drinks.total_litres_of_pure_alcohol.sort_values(ascending=False)[0:10]


# BONUS: sort 'users' by 'occupation' and then by 'age' (in a single command)
users.sort_values(['occupation','age'],ascending=False)
...
desc = ufo.describe(include='all')      # describe all columns
ufo.mode()
